...
if 'tm_helices_uniprot' in st.session_state and st.session_state.tm_helices_uniprot:
    tm_helices_uniprot = st.session_state.tm_helices_uniprot
    tm_data = []
    for start, end in tm_helices_uniprot:
        tm_data.append({
            "Start": start,
    ...
    df_tm_annotations = pd.DataFrame(tm_data)
    ...
    updated_tm_data = []
    for i, row in df_tm_annotations.iterrows():
        index = i + 1
        with col3:
            start = st.number_input(f"Start {index}", value=row["Start"], key=f"start_{index}")
        with col4:
            end = st.number_input(f"End {index}", value=row["End"], key=f"end_{index}")
        region = row["Region"]
        
        updated_tm_data.append({
            "Region": region,
            "Start": start,
            "End": end,
        })

    updated_df = pd.DataFrame(updated_tm_data)
    updated_df.index = range(1, len(updated_df) + 1)
    with col1:
        st.write(updated_df.to_html(index=True, escape=False), unsafe_allow_html=True)
        st.session_state.tm_helices_uniprot = [(row['Start'], row['End']) for index, row in updated_df.iterrows()]

    pred_manual = ['g'] * len(st.session_state.sequence)
    for start, end in updated_df[['Start', 'End']].values:
        for i in range(start - 1, end):
            pred_manual[i] = 'M'

    show_pdb_button = st.button('Show PDB with manual annotations')

    if show_pdb_button and pred_manual is not None:
        sequence = st.session_state.get("sequence", None)
        pdb_structure = st.session_state.get("pdb_structure", None) or fetch_pdb_structure(default_pdb)
