...
try:
    ...
    if hasattr(pdb_unpID, 'pdb_unpID'):
        print("Successfully imported pdb_unpID module.")
...
st.sidebar.markdown("[PDBe ligand viewer](#2D-viewer)")
...
if process_button and pdb_input:
    ...
    # Clean up any .cif files in the scripts directory
    scripts_dir = os.path.join(script_dir, "scripts")
    for file in glob.glob(os.path.join(scripts_dir, "*.cif")):
        try:
            os.remove(file)
        except Exception as e:
...
if st.session_state.processing_complete and st.session_state.df_interactions is not None:
    ...
    if not df_display.empty:
        ...
        df_csv = df_display.drop(columns=["Ligand Position"], errors="ignore")
        ...
        with col1:
            ...
            if selected_ligand != "All Ligands":
                ...
                st.markdown("#### PDBe ligand viewer")
                ...
                <body>
                    <div style="width: 600px; height: 400px; position: top;">
